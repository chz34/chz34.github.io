---
title: PyTorch Inductor 图编译流程深度解析
date: 2026-04-22 00:00:00 +0800
categories: [AI框架, PyTorch]
tags: [PyTorch, Inductor, torch.compile, Triton, 编译器]
---
...
![整体架构概览](/assets/img/inductor-pipeline/diagram1-arch.svg){: style="width:100%; height:auto"}
...
<img src="/assets/img/inductor-pipeline/diagram2-callchain.svg" alt="编译入口与调用层次" style="height: auto">
</div>
...
> **关键设计**：`pre_grad` 和 `joint_graph` 两个阶段以**回调方式**注入 AOTAutograd（`pre_grad_passes=fn`、`partition_fn=fn`），在 `aot_autograd` 内部执行，完全早于 `codegen_and_compile` 的调用。后三个阶段（post_grad、Lowering、Scheduler）则在 Inductor 自身的 `codegen_and_compile` 闭环内直接执行。
>
> **pre_grad 的时机细节**：pre_grad 的实际执行点由 `resolve_pre_grad_pass_timing()` 决定，分为两种情况——若自定义 pass 未实现 `uuid()`（无法参与 cache key 计算），则在 cache 查询**之前**运行（`"early"`）；否则默认在 cache miss 之后、`aot_stage1_graph_capture` 构建 joint graph **之前**运行（`"late"`）。两种情况下 pre_grad 都早于 autograd 展开，保证了其"pre-autograd"的语义。
...
**触发时机**：AOTAutograd 内部，构建 joint graph 之前。具体执行点由 `resolve_pre_grad_pass_timing()` 决定：若自定义 pass 无 `uuid()`（`"early"`），则在 AOTAutograd cache 查询**之前**执行；否则（`"late"`，默认）在 cache miss 之后执行。两种情况都早于 autograd 展开。
...
<div style="overflow-x: auto">
<img src="/assets/img/inductor-pipeline/diagram3b-jointgraph.svg" alt="Joint Graph Pass 与图分割" style="height: auto">
</div>
...
<div style="overflow-x: auto">
<img src="/assets/img/inductor-pipeline/diagram3-irnodes.svg" alt="IR 节点层次结构" style="height: auto">
</div>
...
![Scheduler 初始化流程](/assets/img/inductor-pipeline/diagram4-scheduler.svg){: style="width:100%; height:auto"}

#### 融合决策逻辑

融合分为**竖向融合**（producer → consumer）和**横向融合**（独立节点共享读取），`can_fuse()` 按以下顺序检查 gate：

![融合决策逻辑](/assets/img/inductor-pipeline/diagram5-fusion.svg){: style="width:100%; height:auto"}
...
<div style="overflow-x: auto">
<img src="/assets/img/inductor-pipeline/diagram6-injection.svg" alt="可注入接口全景" style="height: auto">
</div>
...
<div style="overflow-x: auto">
<img src="/assets/img/inductor-pipeline/diagram7-torchmlir.svg" alt="外部编译器集成：torch-mlir" style="height: auto">
</div>
...
| | Inductor | torch-mlir |
|--|---------|-----------|
| 接管位置 | Dynamo 捕获 FX graph 后，在 Inductor 内部处理 | 路径一：Dynamo 捕获后直接接管（绕过 Inductor）；路径二：`torch.export` 之后接管 |
...
<div style="overflow-x: auto">
<img src="/assets/img/inductor-pipeline/diagram8-dataflow.svg" alt="编译阶段与数据流" style="height: auto">
</div>
