...
flowchart TD
    ...
    C --> D["_compile_fx_main()\n核心编排：注册 pre_grad/partition 回调"]

    D -->|"aot_autograd(\n  pre_grad_passes=run_pre_grad_passes,\n  partition_fn=partition_fn,\n  fw_compiler=...\n)"| AOT

    subgraph AOT["aot_autograd 内部（torch/_functorch/aot_autograd.py）"]
        direction TB
        P1["① run_pre_grad_passes(mod)\npre_grad 在此执行\n— early: cache 查询前\n— late: cache miss 后（默认）"] --> P2
        P2["AOTAutogradCache.try_load()\ncache 查询"] -->|cache miss| P3
        P3["aot_stage1_graph_capture()\n构建 joint graph\n（forward trace + autograd 展开）"] --> P4
        P4["aot_stage2_compile()\n调用 partition_fn + fw/bw compiler"]
        P4 -->|"② partition_fn 内\njoint_graph_passes + 图分割"| PF["joint graph passes\n→ fw_graph / bw_graph"]
        PF -->|fw_compiler 回调| FW
        PF -->|bw_compiler 回调| BW
    end

    subgraph FW["Forward 编译链"]
        direction TB
        E["compile_fx_forward()"] --> G["compile_fx_inner()"]
        G --> H["_compile_fx_inner()\nFX cache 查询"]
        H --> I["fx_codegen_and_compile()"]
        I --> J["codegen_and_compile()\n实际编译主体"]
        J -->|"③ post_grad"| M["_recursive_post_grad_passes()"]
        J -->|"④ Lowering"| N["GraphLowering.run()"]
        J -->|"⑤ Scheduler + Codegen"| O["graph.codegen()\n→ Scheduler → kernel"]
    end

    subgraph BW["Backward 编译链（结构同上）"]
        direction TB
        F["compile_fx_backward()"] --> G2["compile_fx_inner()"]
        G2 --> J2["codegen_and_compile()"]
        J2 -->|"③④⑤ 同 Forward"| O2["post_grad → Lowering\n→ Scheduler → kernel"]
    end

    style P1 fill:#e8f4e8,stroke:#5a9a5a
    style PF fill:#e8f4e8,stroke:#5a9a5a
    style M fill:#e8f4e8,stroke:#5a9a5a
    style N fill:#fef9e7,stroke:#c8a030
    style O fill:#fdecea,stroke:#c85a5a
    style O2 fill:#fdecea,stroke:#c85a5a
    style AOT fill:#dff0d8,stroke:#7ab87a
    style FW fill:#fdf6e3,stroke:#c8a84b
    style BW fill:#fdf6e3,stroke:#c8a84b
```
...
flowchart LR
    ...
    B -->|"aot_autograd 内部\n① pre_grad_passes\n② joint graph 构建\n③ joint_graph_passes + partition"| E["分离 FX Graph\n(post-autograd ATen)"]
